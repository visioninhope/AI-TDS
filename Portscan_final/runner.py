...
def network_run(duration_seconds):
    ...
    process = subprocess.Popen(["cmd", "/c", commands], shell=True, stdout=subprocess.DEVNULL,
                               stderr=subprocess.DEVNULL)
    ...
    try:
        ...
        for i in loaded_model.predict(array_pred):
            if i == 1:
                count1 += 1
                if count1 == 3:  # Minimum 3 packets in the csv have to be flagged (having more PSH ON packets might
                    # result in this
                    mal = 2
    except:
        pass
    ...
    if mal == 1:
        ...
        print(
            "Mostly normal network activity, however Our AI/ML modal suspects a small scale PortScan activity in ur "
            "network.")
        return 2
    else:
